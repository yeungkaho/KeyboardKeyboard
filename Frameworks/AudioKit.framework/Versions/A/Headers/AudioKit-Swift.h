...
SWIFT_CLASS("_TtC8AudioKit19AKBooster2AudioUnit")
@interface AKBooster2AudioUnit : AKAudioUnitBase
...
///
- (nonnull instancetype)initWithFile:(AKAudioFile * _Nonnull)file startPoint:(uint32_t)startPoint endPoint:(uint32_t)endPoint rate:(double)rate volume:(double)volume maximumSamples:(NSInteger)maximumSamples completionHandler:(void (^ _Nonnull)(void))completionHandler OBJC_DESIGNATED_INITIALIZER;
/// Function to start, play, or activate the node, all do the same thing
- (void)start;
/// Function to stop or bypass the node, both are equivalent
- (void)stop;
